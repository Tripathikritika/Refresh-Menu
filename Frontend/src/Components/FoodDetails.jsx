...
export default function FoodDetails () {
   ...
   console.log(query , locationSearch)
    ...
    const handleAddons = (item , e) => {
      ...
      const handleClose = () => {
          const data = { title : foodItem.title , amount : foodItem.amount , type : foodItem.type , qty : 1 , addons : [] }
          let quantity = cartList.find((res) => res.title === foodItem.title )
          if( quantity ){
              quantity.qty = quantity.qty+1
              cartList = cartList.map((res) => res.title === foodItem.title ? quantity : res)
          }
          else{
    ...
    const handleAddItem = ( ) => {        
        const data = { title : foodItem.title , amount : Number(foodItem.amount) + Number(addonsTotalPrice) , type : foodItem.type , qty : 1 , addons : addons}
        let quantity = cartList.find((res) => res.title === foodItem.title )
        if( quantity){
            quantity.qty = quantity.qty+1
            cartList = cartList.map((res) => res.title === foodItem.title ? quantity : res)
        ...
        <div style={{ width: "100%",height: "100%",backgroundColor: "#eeeeee",}}>
            <div style={modalStyle} className={classes.paper}>
                <div className={styles.modalLayout}>
                    <h3 id="simple-modal-title" className={styles.modalTitle}> {foodItem.title}</h3>
                    <h4><i className="fas fa-times text-secondary" onClick = {handleClose} ></i></h4> 
                </div>
                <p id="simple-modal-description">
                    <div className={styles.addonsTitle}> AddOns</div>
                    {  
                       foodItem.addons===true && foodItem.addons_value.map((items) => (
        ...
        <>
                                        ...
                                        <div className="col-7">
                                            <div className="row ">
                                                ...
                                                <div className="col-9 text-left p-4" >
                                                    <div data-toggle="modal" className="d-flex font-weight-bold" data-target="#mapLocation">
                                                        <div className="text-secondary"> Deliver to:</div>
                                                        <span style={{color:'#e85826'}}>{getLocation}</span>
                                                        <img src="/downarrow.svg" alt="downarrow.svg" />
                                                    </div>
                                                    <div>
                                                <div className="modal fade" id="mapLocation"
                                                    style={{
                                                    width: "100%",
                                                    height: "100%",
                                                    ...
                                                    <div className="modal-dialog">
                                                        <div className="modal-content">
                                                            ...
                                                            <div className={styles.buttonGroup}>
                                                                <button type="button" className={`btn rounded-pill ${styles.button}`}>
                                                                    Bengaluru{" "}
                                                                </button>
                                                                <button type="button" className={`btn rounded-pill ${styles.button}`}
                                                                >
                                        ...
                                        <div className="col-5">
                                            ...
                                            <div className={`col-4 ${styles.freshClub} p-4`} >
                                                <Link to='/offers' target='_Blank' style={{display:'flex',textDecoration:'none',color: '#4a4a4a'}}>
                                                    <img src="/Clubs.svg" className="img-fluid pr-1" width="20px" alt="clubs" />
                                            ...
                                            <div className="col-2 text-center" style={{backgroundColor: cartList.length===0 ? "white" : "#f3c920"}}>
                                                <IconButton aria-label="open drawer" edge="end" style={{margin : '0px auto'}}  onClick={handleDrawerOpen} className={`mt-3 ${clsx(open)}`}>
                                                    <img src="/carticon.svg" alt="carticon.svg"/>
                                                    <div className={styles.cartLength}><h5>{cartList.length}</h5></div>
                                                </IconButton>
                                            </div>
          ...
          <div >
            ...
            <div className={styles.cartCount}>
                {
                  cartList.length === 0 ? "" :  <div>{cartList.length} Items</div>
                }
            </div>
